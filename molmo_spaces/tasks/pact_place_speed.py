...
PENDANT_GEOM_PREFIX = "pact_clutter_mount_v106_"
...
def speed_schedule(primitives: Sequence[Any]) -> list[dict[str, Any]]:
    ...
    for primitive_index, primitive in enumerate(primitives):
        if not isinstance(primitive, TCPMoveSequence):
            continue
        for segment_index, segment in enumerate(getattr(primitive, "_move_segments", None) or []):
            out.append(
                {
...
def schedule_sha256(primitives: Sequence[Any]) -> str:
    return hashlib.sha256(
        json.dumps(speed_schedule(primitives), sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()
...
def plan_signature(primitives: Sequence[Any]) -> dict[str, Any]:
    ...
    for primitive_index, primitive in enumerate(primitives):
        if not isinstance(primitive, TCPMoveSequence):
            entries.append(
                {"primitive_index": int(primitive_index), "type": type(primitive).__name__}
            )
            continue
        for segment_index, segment in enumerate(getattr(primitive, "_move_segments", None) or []):
            entries.append(
                {
                    ...
                    "name": str(segment.name),
                    "speed_m_s": round(float(segment.speed), 9),
                    "start_position_m": [round(float(v), 9) for v in segment.start_pose[:3, 3]],
                    "end_position_m": [round(float(v), 9) for v in segment.end_pose[:3, 3]],
                    "start_rotation": [
                        round(float(v), 9)
                        for v in np.asarray(segment.start_pose[:3, :3]).reshape(9)
                    ],
                    "end_rotation": [
                        round(float(v), 9) for v in np.asarray(segment.end_pose[:3, :3]).reshape(9)
                    ],
                }
            )
...
def apply_initial_free_space_speed_cap(primitives: Sequence[Any]) -> dict[str, Any]:
    ...
    if changed and n_changed != 1:
        raise SpeedAmendmentError(f"the cap must alter exactly one segment, altered {n_changed}")
    return {
