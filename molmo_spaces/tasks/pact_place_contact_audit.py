...
def classify_contact(pair: dict[str, Any]) -> str:
    blob = " ".join(
        str(pair.get(key, "")) for key in ("geom1", "geom2", "body1", "body2", "root1", "root2")
    )
...
class PactPlaceContactAudit:
    ...
    def __init__(self) -> None:
        self._retain_contact_frames = os.environ.get("PACT_CONTACT_AUDIT_SUMMARY_ONLY") != "1"
        self.reset()
            ...
            self._pairs_by_step.append(
                {
                    ...
                    "pairs": pairs,
                    "contact_classes": {key: int(frame_counts[key]) for key in CONTACT_CLASSES},
                }
            )
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "inbound_hazard_contact_frames": self._phase_frames_with["inbound"]["hazard_bar"],
            "outbound_hazard_contact_frames": self._phase_frames_with["outbound"]["hazard_bar"],
